...
use sync_core::{sync_capnp, LInstant};
...
async fn handle_authed_recv(
...

    {
        ...
        match req.which() {
            ...
            Ok(Which::Aware(aware)) => {
                ...
                match values.len() {
                    ...
                    1 => {
                        ...
                        let (aware_client, entry) = sync_core::aware::read_entry(reader)?;
            ...
            Ok(Which::Delta(delta)) => {
                ...
                let delta = delta?;
                let delta_ts = sync_core::read_delta_ts(delta)?;
                client.merge(delta)?;
    ...
    if let Some(delta_ts) = should_confirm_delta {
        if last_save_failed.load(atomic::Ordering::Acquire) {
            ...
            let mut b = MessageBuilder::new_default();
            sync_core::write_confirm_delta(b.init_root(), delta_ts);
...
impl From<sync_core::Error> for ErrReply {
    fn from(err: sync_core::Error) -> Self {
