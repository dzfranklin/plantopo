...
func TestBasicSession(t *testing.T) {
	...
	wantWelcomeC1 := OutgoingSessionMsg{
		Aware: &map[uuid.UUID]schema.Aware{
			c1.ClientId: blankAwareFor(c1.ClientId),
		},
	...
	wantWelcomeC2 := OutgoingSessionMsg{
		Aware: &map[uuid.UUID]schema.Aware{
			c1.ClientId: blankAwareFor(c1.ClientId),
			c2.ClientId: blankAwareFor(c2.ClientId),
		},
		Change: &emptyChangeset,
	}
	...
	c1.Incoming <- IncomingSessionMsg{
		Seq: 1,
		Aware: schema.Aware{
			Trusted:          schema.TrustedAware{ClientId: c1.ClientId},
...
func blankAwareFor(clientId uuid.UUID) schema.Aware {
	return schema.Aware{
