...
func TestSingleSimple(t *testing.T) {
	...
	_, err := subject.Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
	require.NoError(t, err)

	_, err = subject.Connect(ctx, mapB, uuid.New(), noopOutgoingChan())
	require.NoError(t, err)
...
func TestLocks(t *testing.T) {
	...
	_, err := server0.Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
	require.NoError(t, err)

	_, err = server1.Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
...
func TestClosesEmptySession(t *testing.T) {
	...
	conn0, err := server0.Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
	require.NoError(t, err)
...
func TestDisconnectReleasesLocks(t *testing.T) {
	...
	_, err := s.subjects[1].Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
	require.NoError(t, err)

	_, err = s.subjects[0].Connect(ctx, mapA, uuid.New(), noopOutgoingChan())
