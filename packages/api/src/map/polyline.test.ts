// From <https://github.com/valhalla/web-app/blob/9d0f14372c18b60fc1da015be75e6d7d565ce2d6/src/utils/polyline.spec.ts> (MIT)
...
describe("decode", () => {
  ...
  it("should handle different precision values", () => {
    const encoded = "_";
    const resultPrecision5 = decodePolyline(encoded, 5);
    const resultPrecision6 = decodePolyline(encoded, 6);
    const resultPrecision7 = decodePolyline(encoded, 7);
  ...
  it("should handle very small precision", () => {
    const encoded = "_";
    const result = decodePolyline(encoded, 1);
  ...
  it("should handle high precision", () => {
    const encoded = "_";
    const result = decodePolyline(encoded, 10);
  ...
  it("should handle invalid input gracefully", () => {
    ...
    const resultNull = decodePolyline(null, 6);
    // @ts-expect-error - Testing with invalid input types
    const resultUndefined = decodePolyline(undefined, 6);
    // @ts-expect-error - Testing with invalid input types
    const resultNumber = decodePolyline(123, 6);
  ...
  it("should handle invalid precision values", () => {
    ...
    const resultNegative = decodePolyline(encoded, -1);
    // @ts-expect-error - Testing with invalid precision types
    const resultNull = decodePolyline(encoded, null);
    const resultUndefined = decodePolyline(encoded, undefined);
  ...
  it("should handle very long encoded strings", () => {
    ...
    const result = decodePolyline(longEncoded, 6);
