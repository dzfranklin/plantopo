// Standalone child-process entry point. Imports sharp (conflicts with canvas).
// Protocol: newline-delimited JSON on stdin/stdout.
// stdin:  StartMsg | CancelMsg
// stdout: ResultMsg | ErrorMsg
...
export type MsgToWorker = StartMsg | CancelMsg;
export type MsgFromWorker = ResultMsg | ErrorMsg;

const inFlight = new Map<string, AbortController>();

const rl = createInterface({ input: process.stdin, crlfDelay: Infinity });

function send(msg: MsgFromWorker): void {
  process.stdout.write(JSON.stringify(msg) + "\n");
}

rl.on("line", line => {
  const msg = JSON.parse(line) as MsgToWorker;
  ...
  const { id, points, accessScopes } = msg;

  const abortController = new AbortController();
  inFlight.set(id, abortController);

  getElevations(points, accessScopes, {
    signal: abortController.signal,
  })
    .then(result => send({ type: "result", id, result }))
    .catch(err => send({ type: "error", id, err: String(err) }))
...
function selectSource(points: Point2[], accessScopes: string[]): DEMSource {
...
async function getElevations(
  ...
  accessScopes: string[],
  {
    tileProvider = fetchTile,
    signal,
  }: { tileProvider?: TileFetcher; signal?: AbortSignal } = {},
  ...
  await Promise.all(
    Array.from(uniqueTiles.values()).map(async ({ x, y }) => {
      ...
      const buf = await tileProvider(source.urlTemplate, z, x, y, signal);
      if (buf === null) return;

      signal?.throwIfAborted();

      const data = await sharp(buf).ensureAlpha().raw().toBuffer();
      decodedTiles.set(key, data);
    }),
  );
  ...
  return {
    data,
    meta: { sources: [source.urlTemplate.replace("{z}", z.toString())] },
  };
}
